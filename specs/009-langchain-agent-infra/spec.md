...
1. Provides a **LangGraph-based agent runtime** that wraps configured AI tools (starting with Claude Code via Anthropic API)
...
- [x] Agent wraps Claude (via @langchain/anthropic) using credentials from existing Settings.agent configuration
...
| `application/ports/output/`       | **Extended** — New port interfaces          | IAgentRunner, ITaskManager, IAgentRegistry |
