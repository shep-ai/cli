...
```typescript
enum ArtifactCategory { PRD = 'PRD', API = 'API', Design = 'Design', Other = 'Other' }
```

### ArtifactFormat

```typescript
enum ArtifactFormat { Markdown = 'md', Text = 'txt', Yaml = 'yaml', Other = 'Other' }
```

### ArtifactState

```typescript
enum ArtifactState { Todo = 'Todo', Elaborating = 'Elaborating', Done = 'Done' }
```

### RequirementType

```typescript
enum RequirementType { Functional = 'Functional', NonFunctional = 'NonFunctional' }
```

### ResearchState

```typescript
enum ResearchState { NotStarted = 'NotStarted', Running = 'Running', Finished = 'Finished' }
```

### MessageRole

```typescript
enum MessageRole { Assistant = 'assistant', User = 'user' }
```
...
```typescript
enum AgentAuthMethod { Session = 'session', Token = 'token' }
```
...
```typescript
enum PrStatus { Open = 'Open', Merged = 'Merged', Closed = 'Closed' }
```

### CiStatus

```typescript
enum CiStatus { Pending = 'Pending', Success = 'Success', Failure = 'Failure' }
```
