...
function parsePlanPhase(raw: Record<string, unknown>): PlanPhase {
  return {
    ...
    taskIds: ensureArray<string>(raw.taskIds),
  };
}
...
/**
 * Parse a spec.yaml string into a FeatureSpec object.
 */
export function parseSpecYaml(content: string): FeatureSpec {
...
/**
 * Parse a research.yaml string into a ResearchSpec object.
 */
export function parseResearchYaml(content: string): ResearchSpec {
  const raw = loadYaml(content);
  const base = extractBaseFields(raw);
...
/**
 * Parse a plan.yaml string into a PlanSpec object.
 */
export function parsePlanYaml(content: string): PlanSpec {
  const raw = loadYaml(content);
  const base = extractBaseFields(raw);
...
/**
 * Parse a tasks.yaml string into a TasksSpec object.
 */
export function parseTasksYaml(content: string): TasksSpec {
  const raw = loadYaml(content);
  const base = extractBaseFields(raw);
