...
describe('getFeatureFlags', () => {
  ...
  it('returns DB values when settings has featureFlags', () => {
    ...
    mockGetSettings.mockReturnValue({
      featureFlags: { skills: true, envDeploy: false, debug: true, databaseBrowser: false },
    });
  ...
  it('debug flag returns false when not in DB (no env var fallback)', () => {
    mockHasSettings.mockReturnValue(true);
    mockGetSettings.mockReturnValue({
      featureFlags: { skills: false, envDeploy: false, debug: false, databaseBrowser: false },
    });
...
describe('featureFlags (backward-compatible const)', () => {
  ...
  it('exposes skills via getter that calls getFeatureFlags', () => {
    mockHasSettings.mockReturnValue(true);
    mockGetSettings.mockReturnValue({
      featureFlags: { skills: true, envDeploy: false, debug: false, databaseBrowser: false },
    });
  ...
  it('exposes envDeploy via getter', () => {
    mockHasSettings.mockReturnValue(true);
    mockGetSettings.mockReturnValue({
      featureFlags: { skills: false, envDeploy: true, debug: false, databaseBrowser: false },
    });
  ...
  it('exposes debug via getter', () => {
    mockHasSettings.mockReturnValue(true);
    mockGetSettings.mockReturnValue({
      featureFlags: { skills: false, envDeploy: false, debug: true, databaseBrowser: false },
    });
