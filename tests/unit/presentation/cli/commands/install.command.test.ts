...
vi.mock('../../../../../src/infrastructure/services/tool-installer/tool-metadata.js', () => ({
  TOOL_METADATA: {
    ...
    windsurf: {
      binary: 'windsurf',
      packageManager: 'apt',
      commands: { linux: ['sh', '-c', 'sudo apt-get install -y windsurf'] },
      timeout: 300000,
      documentationUrl: 'https://docs.windsurf.com/windsurf/getting-started',
      verifyCommand: ['windsurf', '--version'],
      notes: 'Windsurf - AI-powered code editor by Codeium',
    },
    zed: {
      binary: 'zed',
      packageManager: 'curl',
      commands: { linux: ['sh', '-c', 'curl -f https://zed.dev/install.sh | sh'] },
      timeout: 300000,
    ...
    antigravity: {
      ...
      commands: { linux: ['sh', '-c', 'echo "Download from https://antigravity.google/download"'] },
      timeout: 300000,
      documentationUrl: 'https://codelabs.developers.google.com/getting-started-google-antigravity',
      verifyCommand: ['agy', '--version'],
      autoInstall: false,
      notes: 'Google Antigravity - AI-powered agentic development platform',
    },
    'cursor-cli': {
      binary: 'cursor',
      packageManager: 'curl',
      commands: { linux: ['sh', '-c', 'curl https://cursor.com/install -fsS | bash'] },
      timeout: 300000,
      documentationUrl: 'https://cursor.com',
      verifyCommand: ['cursor', '--version'],
      notes: 'Cursor CLI - AI-powered code editor CLI',
    },
    'claude-code': {
      binary: 'claude',
      packageManager: 'curl',
      commands: { linux: ['sh', '-c', 'curl -fsSL https://claude.ai/install.sh | bash'] },
      timeout: 300000,
