...
describe('SQLiteSettingsRepository', () => {
  ...
  describe('feature flags', () => {
    ...
    it('should store feature flag booleans as INTEGER 0/1', async () => {
      ...
      const row = db
        .prepare(
          'SELECT feature_flag_skills, feature_flag_env_deploy, feature_flag_debug, feature_flag_github_import, feature_flag_adopt_branch, feature_flag_git_rebase_sync, feature_flag_react_file_manager, feature_flag_database_browser FROM settings WHERE id = ?'
        )
      ...
      expect(row.feature_flag_database_browser).toBe(0);
    });
  });
